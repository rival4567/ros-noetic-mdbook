...
class Edrone():
	...
	def process_command(self):
		...
		if self.curnt_task == "m":
			self.long = 71.99983191056211
			self.lat = self.x_to_lat(self.lat_to_x(19.0007046575)-80)
		elif self.curnt_task == "l":
			self.move_xy(0, -10)
		elif self.curnt_task == "f":
			self.lat = self.x_to_lat(self.lat_to_x(19.0007046575)-80)

	def detect_collision(self):
		self.curnt_range = self.ranges[0]
		if self.curnt_task == "l" or self.curnt_range > 20 or self.curnt_range == float('inf') or self.curnt_range < 2 or self.curnt_task == "null":
			return
		if self.curnt_range <= abs(self.lat_to_x(self.lat)-self.lat_to_x(self.curnt_pos[1])):
			self.lat = self.curnt_pos[1]
			self.destination_reached = False
			print("collision!")
			if self.curnt_task == "m":
	...
	def main(self):  # this function calls the all necessary function
		...
		if (self.height_reached == True and self.destination_reached == True):
			self.process_command()
		print(self.task, self.curnt_task)
		print(self.lat, self.long, self.height)
		print(self.curnt_range)
